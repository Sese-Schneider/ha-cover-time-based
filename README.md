...
**Tilt on a wrapped cover.** The **Inline** and **Sequential** tilt modes can drive the wrapped cover's normal open / close commands, so they work on any wrapped cover regardless of whether it reports tilt support. If an **Inline** wrapped cover advertises native `set_cover_tilt_position`, tilt-position, tilt-open, tilt-close and post-travel tilt-restore commands are forwarded to the wrapped cover instead of being simulated with timed main-motor runs. When the wrapped cover later reports `current_tilt_position`, the integration snaps its tilt tracker to that reported value.

**Tilt follows travel (`tilt_follows_travel`).** For wrapped **Inline** covers with native `set_cover_tilt_position`, this option controls only the tracker display during travel. Leave it **on** for venetian-style covers whose slats physically sweep to the direction endpoint while the cover travels (closed on the way down, open on the way up). Turn it **off** if the wrapped cover's slats do not follow travel direction; the tilt display will hold its current value during travel and sync again from the wrapped cover's reported tilt after it settles. This option is hidden when native tilt forwarding cannot be used, and it is ignored by **Reports commands, not endpoints** covers.

Only the **Separate tilt motor** mode requires the wrapped cover to expose its own tilt open / close commands, so it is offered only when the wrapped entity reports native tilt support.
