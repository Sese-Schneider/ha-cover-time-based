...
async def test_set_known_position_stale_last_command_no_spurious_stop(
    ...
    something to actually be moving. A bare ``_handle_stop`` (calibration
    start, raw_command) halts ``travel_calc`` but never touches
    ``_last_command``, so a stale command can outlive the movement it
    described. A following ``set_position`` then sees a "direction change"
    against nothing moving; without the "is anything moving" conjunct this
    pulses an extra stop at an idle motor -- on toggle-opposite hardware a
    movement command that runs the cover to its endpoint (#153-class
    hazard)."""
    cover = make_cover(control_mode="toggle_opposite", recalibrate_before_position=True)
    ...
    assert not cover.travel_calc.is_traveling(), "must be halted by the reset"
    assert cover._last_command == SERVICE_CLOSE_COVER, (
        "_handle_stop must not touch _last_command -- that's the setup"
    )
