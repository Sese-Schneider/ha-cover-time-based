...
- **New per-cover setting — "Direction change delay"** ([#153](https://github.com/Sese-Schneider/ha-cover-time-based/issues/153)): reversing a moving cover is never a single step — the motor is stopped, given a moment to come to rest, and only then driven the other way. That settle gap was a hardcoded 1.0 second, but the time a motor needs to come to rest varies by hardware. When it is too short the reverse command arrives while the motor is still coming to rest and is simply **ignored**, but the integration has already started counting the new movement — so the cover sits motionless while its reported position ticks along to the target and desyncs from reality. The symptom is a position change that requires a reversal (the cover is opening towards 75% and you select 25%) leaving the cover parked while Home Assistant animates down to 25%. Reported for an **Aqara T2** driving sliding blinds and confirmed against the hardware, where setting the gap to `0` leaves the blind standing still — the reverse command lands milliseconds after the stop and is simply ignored — while the same motor reverses correctly on the 1.0s default. A new per-cover option, **Direction change delay** (default **1.0s**, the unchanged behaviour), makes that gap configurable — raise it until reversals take reliably, using at least the time the motor visibly needs to come to a complete rest. It applies to every control mode, since a reversal is reachable from all of them, and also governs the tilt-restore reversal on covers whose slats share the travel motor. `0` remains a legitimate "no settle gap". Configurable from the card's Timing tab (EN/PT/PL) and as the `direction_change_delay` YAML option.
