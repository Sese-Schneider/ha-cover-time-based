...
Currently supported languages: English (`en`), Portuguese (`pt`), Polish (`pl`), German (`de`), Italian (`it`), Dutch (`nl`), French (`fr`). This line is the canonical list — everything else derives from the catalogues themselves, so adding a language only touches it here.
...
1. Copy `custom_components/cover_time_based/translations/en.json` to `custom_components/cover_time_based/translations/es.json`.
2. Translate every value in `es.json`. Keep the keys exactly the same — Home Assistant looks them up by name.
...
2. Add an `es:` entry mirroring the existing non-English blocks. Use the `EN` object above it as the master list of keys: copy every key across and translate its value.
